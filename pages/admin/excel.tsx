...
export default function ExcelManager() {
  ...
  const handleImport = async (type: 'categories' | 'products') => {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      try {
        ...
        await axios.post(`http://34.205.64.185:8000/api/admin/excel/${type}/import`, formData, {
          headers: { 
  ...
  const handleExport = async (type: 'categories' | 'products') => {
    ...
    try {
      const token = localStorage.getItem("token");
      const res = await axios.get(`http://34.205.64.185:8000/api/admin/excel/${type}/export`, { 
        responseType: 'blob',
        headers: {
          Authorization: `Bearer ${token}`,
        },
