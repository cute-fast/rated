...
export default function ProductManager() {
  ...
  const fetchProducts = async (q: string, pageNum = 1) => {
    setLoading(true);
    try {
      const res = await axios.get(`https://api.rated.xyz/api/search/products`, {
  ...
  const handleModalSubmit = async (values: any) => {
    try {
      ...
      if (modal?.type === 'add') {
        await axios.post('https://api.rated.xyz/api/admin/products', values, {
          headers:{
        ...
        await axios.patch(`https://api.rated.xyz/api/admin/products/${modal.product.asin}`, values, {
          headers:{
            Authorization: `Bearer ${token}`
          }
        });
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      await axios.delete(`https://api.rated.xyz/api/admin/products/${modal.product.asin}`, {
        headers:{
          Authorization: `Bearer ${token}`
        }
      });
