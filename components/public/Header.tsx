    ...
    const fetchSuggestions = async () => {
        ...
        try {
            const [prodRes, catRes] = await Promise.all([
                axios.get('https://34.205.64.185:8000/api/search/products', { params: { q: searchValue, size: 5 } }).catch((err) => {
                    console.error('Products search error:', err)
                    return { data: [] }
                }),
                axios.get('https://34.205.64.185:8000/api/search/categories', { params: { q: searchValue, size: 5 } }).catch((err) => {
